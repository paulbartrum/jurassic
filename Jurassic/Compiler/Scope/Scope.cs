...
namespace Jurassic.Compiler
{
    ...
    public abstract class Scope
    {
        ...
        /// <param name="generator"> The generator to output the CIL to. </param>
        /// <param name="optimizationInfo"> Information about any optimizations that should be performed. </param>
        /// <param name="fromEvalMethodGenerator"> If the declarations are generated by the <see cref="EvalMethodGenerator"/>. </param>
        internal virtual void GenerateDeclarations(ILGenerator generator, OptimizationInfo optimizationInfo, bool fromEvalMethodGenerator = false)
        {
            ...
            foreach (var variable in this.variables.Values)
            {
                ...
                // However, if this is not done from the EvalMethodGenerator, we may need
                // to reinitialize the variables, for example because the Loop statement
                // may generate code two times. See issue #140.
                if (fromEvalMethodGenerator && variable.Initialized == true)
                    continue;
